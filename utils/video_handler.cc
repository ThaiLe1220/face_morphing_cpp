...
std::vector<std::pair<cv::Mat, int>> extractFramesWithDuration(const std::string &videoPath, int outputResolution, int outputFPS, int start, int duration)
{
    ...
    int startFrameIndex = static_cast<int>(start * videoFPS);
    int endFrameIndex = std::min(startFrameIndex + static_cast<int>(duration * videoFPS), static_cast<int>(cap.get(cv::CAP_PROP_FRAME_COUNT)));
    ...
    int outputHeight = static_cast<int>(outputResolution * aspectRatio);
    ...
    while (cap.isOpened() && cap.get(cv::CAP_PROP_POS_FRAMES) < endFrameIndex)
    {
        ...
        int currentTimestamp = static_cast<int>(currentFrameIndex / videoFPS * 1000);
        ...
        for (int i = 0; i < static_cast<int>(videoFPS / outputFPS) - 1; ++i)
            cap.grab();
    }
