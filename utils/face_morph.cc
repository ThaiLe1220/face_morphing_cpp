...
void morph_triangle(cv::Mat& img1, cv::Mat& img2, cv::Mat& morphed_img, std::vector<cv::Point2f>& t1, std::vector<cv::Point2f>& t2, std::vector<cv::Point2f>& t, float alpha) {
...
cv::Mat mask_out_non_black(cv::Mat& image) {
    ...
    cvtColor(image, grayscale_image, cv::COLOR_BGR2GRAY);

    // Threshold the grayscale image to create a binary mask
    cv::Mat binary_mask;
    threshold(grayscale_image, binary_mask, 1, 255, cv::THRESH_BINARY);

    return binary_mask;
}

cv::Mat add_exclusive_img(cv::Mat img1, cv::Mat img2) {
    cv::Mat img1_copy = img1.clone();

    // Create masks to identify non-black pixels in both images
    cv::Mat non_black_mask1, non_black_mask2;
    cv::cvtColor(img1, non_black_mask1, cv::COLOR_BGR2GRAY);
    cv::cvtColor(img2, non_black_mask2, cv::COLOR_BGR2GRAY);
    cv::threshold(non_black_mask1, non_black_mask1, 1, 255, cv::THRESH_BINARY);
    cv::threshold(non_black_mask2, non_black_mask2, 1, 255, cv::THRESH_BINARY);

    // Find pixels in img1 that are black and pixels in img2 that are not black
    cv::Mat exclusive_pixels;
    cv::bitwise_not(non_black_mask1, non_black_mask1);
    cv::bitwise_and(non_black_mask1, non_black_mask2, exclusive_pixels);

    // Replace pixels in img1 with pixels from img2 where img1 is black and img2 is not black
    img2.copyTo(img1_copy, exclusive_pixels);
...
cv::Mat morph_images(cv::Mat& img1, cv::Mat& img2, std::vector<cv::Point2f>& points1, std::vector<cv::Point2f>& points2, std::vector<std::tuple<int, int, int>>& tri_list, float alpha) {
    ...
    // mouth_list_ids_dlib = [60,61,62,63,64,65,66,67]
    ...
    // cv::add(img1_no_face, morphed_img, result);
    // result = add_exclusive_img(morphed_img, img1_no_face);
    cv::bitwise_not(mask,mask);
    // cv::imshow("mask",mask);
    // cv::imshow("morphed_img",morphed_img);
    // cv::imshow("img1_no_face",img1_no_face);
    cv::seamlessClone(morphed_img, img1, mask, center_face, result_nor,cv::NORMAL_CLONE);
    // cv::imshow("result_nor",result_nor);
    cv::seamlessClone(morphed_img, img1, mask, center_face, result_mix,cv::MIXED_CLONE);
    // cv::imshow("result_mix",result_mix);

    cv::Mat result = selective_alpha_blend(result_nor, result_mix, mask, alpha);
    // cv::addWeighted(result_nor, alpha,result_mix, 1-alpha, 0, result);
    // cv::addWeighted(result, 0.9, img1, 0.1, 0, result);
    // cv::imshow("seamlessClone",result);
    // cv::waitKey(0);
    return result;
}
