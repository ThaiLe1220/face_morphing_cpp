...
int main()
{
    ...
    std::string pathToFirstImage = "img/tam2.jpg";
    std::string pathToSecondImage = "img/taylor4.jpg";
    ...
    // Check if any faces are detected in the second image
    if (dets.empty())
    {
    ...
    // Extract landmarks from the second image
    ...
    for (auto &rect : dets)
    {
        ...
        for (int i = 0; i < 68; i++)
        {
            lmLists2.push_back(cv::Point2f(shape.part(i).x(), shape.part(i).y()));
        }
    }
    ...
    // List of triangle indices for Delaunay triangulation
    ...
    for (int i = 0; i < frames_with_timestamp.size(); i++)
    {
        try
        {
            // Start the clock for measuring frame processing time
            auto start = std::chrono::high_resolution_clock::now();

            std::cout << "Frame: " << i << std::endl;
            cv::Mat image = frames_with_timestamp[i].first;

            // Generate face correspondences between the current frame and the second image
            ...
            // Measure the elapsed time for processing the current frame
